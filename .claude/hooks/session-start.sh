...
if [ -n "${HTTPS_PROXY:-}" ]; then
  # Extract username, password, host and port from proxy URL
  # Format: http://username:password@host:port
  if [[ "$HTTPS_PROXY" =~ ^https?://([^:]+):([^@]+)@([^:]+):([0-9]+) ]]; then
    ...
    # Fallback to old parsing (no auth)
  ...
  # Get no_proxy list and convert to pipe-separated format for Maven
  NO_PROXY_HOSTS=$(echo "${NO_PROXY:-localhost,127.0.0.1}" | tr ',' '|')
